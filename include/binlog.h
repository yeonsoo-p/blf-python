 ...
 */
BLAPI(BLHANDLE)
BLCreateFile(const char* lpFileName, uint32_t dwDesiredAccess);
/**
 * Use this function to open a BL file with the desired access.
 * \param[in] lpFileName file name to create/open
 * \param[in] dwDesiredAccess access type, GENERIC_READ or GENERIC_WRITE
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileW(const wchar_t* lpFileName, uint32_t dwDesiredAccess);
/**
 * Use this function to open a BL file with the desired access.
 * \param[in] lpFileName file name to create/open
 * \param[in] dwDesiredAccess access type, GENERIC_READ or GENERIC_WRITE
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileEx(const char* lpFileName, uint32_t dwDesiredAccess, const char* lpServer, const char* lpHost);
/**
 * Use this function to open a BL file with the desired access.
 * \param[in] lpFileName file name to create/open
 * \param[in] dwDesiredAccess access type, GENERIC_READ or GENERIC_WRITE
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileExW(const wchar_t* lpFileName, uint32_t dwDesiredAccess, const wchar_t* lpServer, const wchar_t* lpHost);
#if defined(__cplusplus)
/**
 * Use this function to open a BL file with the desired access.
 * \param[in] lpFileName file name to create/open
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileEx2(const char* lpFileName, uint32_t dwDesiredAccess, const char* lpServer, const char* lpHost, IBLCallback* pCallback);
/**
 * Use this function to open a BL file with the desired access.
 * \param[in] lpFileName file name to create/open
 * \param[in] dwDesiredAccess access type, GENERIC_READ or GENERIC_WRITE
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileEx2W(const wchar_t* lpFileName, uint32_t dwDesiredAccess, const wchar_t* lpServer, const wchar_t* lpHost, IBLCallback* pCallback);
/**
 ...
 */
BLAPI(BLHANDLE)
BLCreateFileEx3W(const wchar_t* lpFileName, uint32_t dwDesiredAccess, const wchar_t* lpServer, void* pProviderInfo, const wchar_t* lpHost, IBLCallback* pCallback);
#endif
/**
 ...
 */
BLAPI(int32_t)
BLWriteObject(BLHANDLE hFile, VBLObjectHeaderBase* pBase);
/**
 ...
 */
BLAPI(int32_t)
BLPeekObject(BLHANDLE hFile, VBLObjectHeaderBase* pBase);
/**
 ...
 */
BLAPI(int32_t)
BLSkipObject(BLHANDLE hFile, VBLObjectHeaderBase* pBase);
/**
 ...
 */
BLAPI(int32_t)
BLReadObject(BLHANDLE hFile, VBLObjectHeaderBase* pBase); /* obsolete - replaced by BLReadObjectSecure */
/**
 * Use this function to read a BL object.
 * \param[in] hFile file handle returned by BLCreateFile with GENERIC_READ access
 * \param[in] pBase pointer to BL object describing the object to be read
 ...
 */
BLAPI(int32_t)
BLReadObjectSecure(BLHANDLE hFile, VBLObjectHeaderBase* pBase, size_t expectedSize);
/**
 ...
 */
BLAPI(int32_t)
BLFreeObject(BLHANDLE hFile, VBLObjectHeaderBase* pBase);
/**
 ...
 */
BLAPI(int32_t)
BLSeekTime(BLHANDLE hFile, uint64_t timeStamp, void* arg, int32_t (*pProgressCallback)(void*, float), uint16_t callbackRate);
 ...
 */
BLAPI(int32_t)
BLSetApplication(BLHANDLE hFile, uint8_t appID, uint8_t appMajor, uint8_t appMinor, uint8_t appBuild);
/**
 ...
 */
BLAPI(int32_t)
BLSetWriteOptions(BLHANDLE hFile, uint32_t dwCompression, uint32_t dwFlags);
/**
 * Use this function to set the measurement start time (old version).
 * \param[in] hFile file handle returned by BLCreateFile with GENERIC_WRITE access
 * \param[in] lpStartTime pointer to the system time structure
 * \return TRUE on success, otherwise FALSE
 */
BLAPI(int32_t)
BLSetMeasurementStartTime(BLHANDLE hFile, const LPSYSTEMTIME lpStartTime);
/**
 * Use this function to set the measurement start time. This is the new version that can handle a timestamp
 * with ns resolution and timezone information.
 * \param[in] hFile file         Handle returned by BLCreateFile with GENERIC_WRITE access
 * \param[in] lpStartTime        Pointer to the system time structure, which is the start time as local time
 * \param[in] startTimeNs        ns part of the start time (part below ms resolution)
 * \param[in] timezoneInfoValid  Pass 0 if no timezone info is available; the next two parameters will be ignored in this case.
 *                               For any other value, the timezone info is considered to be valid.
 * \param[in] utcOffset          Offset of the start time to UTC in minutes, including daylight saving time offset. Values greater than 0 means east of Greenwich.
 * \param[in] isDST              Pass any value != 0 if the local timestamp has daylight saving time, 0 otherwise.
 * \return TRUE on success, otherwise FALSE
 */
BLAPI(int32_t)
BLSetMeasurementStartTime2(BLHANDLE hFile, const LPSYSTEMTIME lpStartTime, uint32_t startTimeNs, uint8_t timezoneInfoValid, int16_t utcOffset, uint8_t isDST);
/**
 ...
 */
BLAPI(int32_t)
BLGetFileStatistics(BLHANDLE hFile, VBLFileStatistics* pStatistics);
/**
 ...
 */
BLAPI(int32_t)
BLGetFileStatisticsEx(BLHANDLE hFile, VBLFileStatisticsEx* pStatistics);
/**
 ...
 */
BLAPI(int32_t)
BLFlushFileBuffers(BLHANDLE hFile, uint32_t dwFlags);
/**
 ...
 */
BLAPI(int32_t)
BLSetCommentAttributeString(BLHANDLE hFile, const wchar_t* lpName, const wchar_t* lpValue);
/**
 ...
 */
BLAPI(int32_t)
BLGetNumCommentAttributes(BLHANDLE hFile, uint32_t* lpNumAttributes);
/**
 ...
 */
BLAPI(int32_t)
BLGetCommentAttributeName(BLHANDLE hFile, uint32_t dwIndex, wchar_t* lpName, uint32_t* lpNameSize);
/**
 ...
 */
BLAPI(int32_t)
BLGetCommentAttributeString(BLHANDLE hFile, const wchar_t* lpName, wchar_t* lpValue, uint32_t* lpValueSize);
/**
 ...
 */
BLAPI(int32_t)
BLPeekTimestamp(BLHANDLE hFile, const VBLObjectHeaderBase* pBase, uint64_t* lpTimestamp);
