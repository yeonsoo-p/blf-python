...
#ifndef BL_WCHAR
#define BL_WCHAR char16_t
#endif
#ifndef BL_LPWSTR
#define BL_LPWSTR BL_WCHAR*
#endif
...
// Macros to access the flags in the deprecated VBLCANFDMessage. Do not use with VBLCANFDMessage64
#define CAN_FD_MSG_EDL(f) (uint8_t)(f & 0x1)
#define CAN_FD_MSG_BRS(f) (uint8_t)((f & 0x2) >> 1)
#define CAN_FD_MSG_ESI(f) (uint8_t)((f & 0x4) >> 2)

// Macros to write the flags in the deprecated VBLCANFDMessage. Do not use with VBLCANFDMessage64
...
typedef struct VBLMacsecStatus_t {
    // Version 0
    VBLObjectHeader mHeader; /* object header */
    uint16_t        mChannel;
    uint16_t        mFlags; /* Valid fields: */
                            /* Bit 0 - mHardwareChannel    */
                            /* Bit 1 - mLinkUpDuration     */
                            /* Bit 2 - Macsec State        */
                            /* Bit 3 - Macsec SCI          */
                            /* Bit 4 - Macsec AN           */
    ...
    uint8_t mAddr[6]; /* Address Part of SCI         */

    uint16_t mPort; /* Port number of the SCI      */

    uint8_t mAN; /* Secure Association Number   */
...
typedef struct VBL10BaseT1SStatus_t {
    // Version 0
    VBLObjectHeader mHeader; /* object header */
    uint16_t        mChannel;

    uint16_t mStatusField; /* Bit 0 - PLCA beacon status */
                           /* Bit 1 - Transmit collision status */
                           /* Bit 2 - Transmit jabber status */
                           /* Bit 3 - Receive in transmit opportunity */
                           /* Bit 4 - Unexpected beacon received */
                           /* Bit 5 - Beacon received before transmit opportunity */
                           /* Bit 6 - PLCA symbols detected with PLCA disabled */
                           /* Bit 7 - End-of-stream delimiter error or jabber */
                           /* Bit 8 - 5B decode error */
                           /* Bit 9-15 - Reserved for further use */
...
/*----------------------------------------------------------------------------
|
| Ethernet 10BaseT1S Statistics
|
-----------------------------------------------------------------------------*/

typedef struct VBL10BaseT1SStatistic_t {
    VBLObjectHeader mHeader; /* object header */
    uint16_t        mChannel;
    uint32_t        mRemoteJabberCount;
    uint32_t        mCorruptedTransmitCount;
    uint16_t        mHardwareChannel;
} VBL10BaseT1SStatistic;
...
#define APPTEXT_TRACELINE_FLAGS(source, showInTrace, textOnly) \
    ((uint32_t)(((uint32_t)(textOnly & 0x01) << 5) | ((uint32_t)(showInTrace & 0x01) << 4) | ((uint32_t)(source & 0x0F))))
...
#define BL_APPID_DSA                    220
#define BL_APPID_ETAS                   221
#define BL_APPID_ACTBLUE_FRANCE         222
#define BL_APPID_INSTA_AUTOMATION       223
#define BL_APPID_FEVSYS                 224
#define BL_APPID_CARIAD                 225
#define BL_APPID_LG_ELECTRONICS         226
#define BL_APPID_ZF                     227
...
typedef struct VBLFileStatisticsEx_t {
    ...
    uint32_t   mObjectCount;            /* number of objects */
    uint32_t   mObjectsRead;            /* number of objects read */
    SYSTEMTIME mMeasurementStartTime;   /* measurement start time as local time */
    SYSTEMTIME mLastObjectTime;         /* last object time as local time */
    uint32_t   mMeasurementStartTimeNs; /* ns part of the measurement start time (everything below ms resolution) */
    uint8_t    mTimezoneInfoValid;      /* 1 if UTC offset and DST flag are valid, 0 otherwise */
    uint8_t    mIsDST;                  /* 1 is measurement start time is a local time with daylight saving time, 0 otherwise */
    int16_t    mUTCOffset;              /* Offset to UTC of the local measurement start time, a value > 0 means each of Greenwich */
    uint32_t   mReserved[16];           /* reserved */
} VBLFileStatisticsEx;
